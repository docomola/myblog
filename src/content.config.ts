...
// Docs collection: Markdown files in src/content/docs/
// Each .md file becomes a page at /docs/相对路径
 ...
 * 
 * Photo albums managed via Markdown files.
 * Frontmatter holds album metadata; body contains images (full-width, flat layout).
 * 
 * Image syntax in body:
 *   ![caption text](https://image-hosting.com/xxx.jpg)
 * 
 * Each .md file becomes /gallery/[slug]
 */
const galleryCollection = defineCollection({
  ...
  schema: z.object({
    ...
    /** Category for filtering (e.g. 摄影, 设计, 开发) */
    category: z.string(),

    /** Display letter on card (single character) */
    letter: z.string(),

    /** Brief description shown on list and detail page */
    description: z.string(),

    /** Cover image URL (图床链接) – used as card background */
    cover: z.string().url().optional(),

    /** Sort order (lower = earlier) */
    order: z.number().default(0),

    /** Whether to show on homepage gallery preview */
    featured: z.boolean().default(false),

    /** Draft mode – hidden from production */
    draft: z.boolean().default(false),
  }),
});
